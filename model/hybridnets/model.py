...
from hybridnets.loss import FocalLoss, FocalLossSeg, TverskyLoss, DiscriminativeLoss
...
class ModelWithLoss(nn.Module):
    def __init__(self, model, debug=False):
        ...
        self.instance_criterion = DiscriminativeLoss(0.5, 1.5, 1.0, 1.0, 0.001)
        self.debug = debug

    def forward(self, imgs, annotations, seg_annot, inst_annot, obj_list=None):
        # _, regression, classification, anchors, segmentation, instance = self.model(imgs)
        segmentation, instance = self.model(imgs)
        k_instance = 0.3
        k_dist = 1.0
        coeff = 1.0
        ...
        if self.debug:
            ...
            var_loss, dist_loss, _ = self.instance_criterion(instance, inst_annot)
        else:
        ...
        instance_loss = (k_instance * var_loss + k_dist * dist_loss) * coeff
...
class SegmentationHead_embd(nn.Sequential):
    def __init__(self, in_channels, out_channels, kernel_size=3, activation=None, upsampling=1):
        ...
        upsampling = nn.UpsamplingBilinear2d(scale_factor=upsampling) if upsampling > 1 else nn.Identity()
        activation = Activation(activation)
        super().__init__(batchNorm, relu, conv2d, upsampling, activation)
