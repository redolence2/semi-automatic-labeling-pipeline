...
from hybridnets.model import SegmentationHead, SegmentationHead_embd
...
class HybridNetsBackbone(nn.Module):
    def __init__(self, num_classes=80, compound_coef=0, seg_classes=1, backbone_name=None, seg_mode=MULTICLASS_MODE, onnx_export=False, **kwargs):
        ...
        self.bifpndecoder_embedding = BiFPNDecoder(pyramid_channels=self.fpn_num_filters[self.compound_coef])
        ...
        self.embedding_head = SegmentationHead_embd(
            in_channels=64,
            out_channels=3,
            activation=None,
            kernel_size=1,
        ...
        self.initialize_decoder(self.bifpndecoder_embedding)
        self.initialize_head(self.segmentation_head)
        self.initialize_head(self.embedding_head)
    ...
    def forward(self, inputs):
        ...
        outputs_embedding = self.bifpndecoder_embedding((p2,p3,p4,p5,p6,p7))

        segmentation = self.segmentation_head(outputs)

        instance = self.embedding_head(outputs_embedding)

        if not self.training:
            instance = self.sigmoid(instance) * 255 # visualize
        ...
        if not self.onnx_export:
            # return features, regression, classification, anchors, segmentation, instance
