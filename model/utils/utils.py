...
def letterbox_inst(combination, new_shape=(384, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
    """缩放并在图片顶部、底部添加灰边，具体参考：https://zhuanlan.zhihu.com/p/172121380"""
    # Resize image to a 32-pixel-multiple rectangle https://github.com/ultralytics/yolov3/issues/232
    img, seg, inst = combination
    shape = img.shape[:2]  # current shape [height, width]
    if isinstance(new_shape, int):
        new_shape = (new_shape, new_shape)
    ...
    if shape[::-1] != new_unpad:  # resize
        ...
        if seg:
            for seg_class in seg:
                seg[seg_class] = cv2.resize(seg[seg_class], new_unpad, interpolation=cv2.INTER_LINEAR)
        inst = cv2.resize(inst, new_unpad, interpolation=cv2.INTER_LINEAR)
    ...
    if seg:
        for seg_class in seg:
            seg[seg_class] = cv2.copyMakeBorder(seg[seg_class], top, bottom, left, right, cv2.BORDER_CONSTANT, value=0)  # add border
        inst = cv2.copyMakeBorder(inst, top, bottom, left, right, cv2.BORDER_CONSTANT, value=0)  # add border

    combination = (img, seg, inst)
    return combination, ratio, (dw, dh)
...
def random_perspective_inst(combination, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
                       border=(0, 0)):
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(0.1, 0.1), scale=(0.9, 1.1), shear=(-10, 10))
    # targets = [cls, xyxy]
    im, seg, inst = combination
    height = im.shape[0] + border[0] * 2  # shape(h,w,c)
    width = im.shape[1] + border[1] * 2
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            for seg_class in seg:
                seg[seg_class] = cv2.warpPerspective(seg[seg_class], M, dsize=(width, height), borderValue=0, flags=cv2.INTER_NEAREST)
            inst = cv2.warpPerspective(inst, M, dsize=(width, height), borderValue=0, flags=cv2.INTER_NEAREST)
        else:  # affine
            im = cv2.warpAffine(im, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            for seg_class in seg:
                seg[seg_class] = cv2.warpAffine(seg[seg_class], M[:2], dsize=(width, height), borderValue=0, flags=cv2.INTER_NEAREST)
            inst = cv2.warpAffine(inst, M[:2], dsize=(width, height), borderValue=0, flags=cv2.INTER_NEAREST)
